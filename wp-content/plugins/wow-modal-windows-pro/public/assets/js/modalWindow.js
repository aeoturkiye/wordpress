...
jQuery(document).ready(function ($) {
    ...
    $.fn.ModalWindow = function (options) {
        ...
        const _default = {
            ...
            cookie: [false, 0, 'cookie-name', 'open'], // [Enable cookie, days, name, timing]
            closeRedirect: [false, '', ''],
        ...
        return this.each(function () {
            ...
            const linkEl = $(content).find('.modal-window__content-box');
            let linkToContent = '';
            ...
            function checkHash() {
                if (settings.hash[0] === false) {
                    return true;
                }
                let hash = window.location.hash;
                if (hash.includes(settings.hash[1])) {
                    return true;
                }
            ...
            function setModalCookie(action = 'open') {
                if (!settings.cookie[0]) {
                    return;
                }

                if (action !== settings.cookie[3]) {
                    return;
                }
            ...
            function checkModalCookie() {
                ...
                for (let i = 0; i < cookieArray.length; i++) {
                    ...
                    // Check if the cookie's name matches the given `cookieName`
                    if (cookie.indexOf(settings.cookie[2] + "=") === 0) {
                        // If it does, return the cookie's value
                        return false;
                    }
                }
            ...
            function setContentFromLink() {
                let api = $('link[rel="https://api.w.org/"]');
                if (linkEl.length && api.length) {
                    const apiHref = api.attr('href');
                    const title = $(linkEl).attr('data-title') ? '1' : '0';
                    const parts = linkToContent.split('/');
                    const slug = parts.filter(Boolean).pop();
                    const postType = $(linkEl).attr('data-post-type') ? $(linkEl).attr('data-post-type') : 'posts';
                    const apiUrl = `${apiHref}wp/v2/${postType}?slug=${slug}`;
            ...
            function openByHover() {
                ...
                $(triggers).on('mouseover', function (event) {
                    ...
                    if ($(this).attr('href') && $(linkEl).length) {
                        linkToContent = $(this).attr('href');
                    }
                    openModalWindow();
                });
            }
            ...
            function closeModalWindow() {
                ...
                redirectOnClose();
                trackingClose();
                setModalCookie('close');
                emptyContentFromLink();
                if ($(fltBtn).length) {
                    $(fltBtn).removeClass('is-paused');
                }
            }

            function emptyContentFromLink() {
            ...
            function closeByClick() {
                ...
                $(content).find('.modal-window__close').on('click keydown', function (e) {
                    if (e.type === 'click' || e.key === 'Enter' || e.key === ' ') {
                        closeModalWindow(); // Функція закриття модалки
                    }
                });
            }
