...
class WOWP_Shortcodes {
	public function __construct() {
		...
		add_shortcode( 'contentBox', [ $this, 'shortcode_link_content' ] );
	}

	public function shortcode_link_content( $atts ): string {
		...
		$title     = ! empty( $atts['title'] ) ? ' data-title="1"' : '';
		$post_type = ! empty( $atts['post_type'] ) ? ' data-post-type="' . esc_attr( $atts['post_type'] ) . '"' : 'posts';

		return '<div class="modal-window__content-box"' . wp_kses_post( $title . $post_type ) . '></div>';
	}
