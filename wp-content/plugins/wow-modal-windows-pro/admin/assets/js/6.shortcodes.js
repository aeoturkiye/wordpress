...
jQuery(document).ready(function ($) {
    ...
    function shortcode_type() {
        const type = $(this).val();
        $('.video-box, .button-box, .iframe-box, .icon-box, .content-box').hide();
        if (type === 'button') {
            $('.button-box').show();
        }
        if (type === 'video') {
    ...
    function builder() {
        ...
        if (type === 'content') {
            ...
            const content_title = $('[data-field="shortcode_content_title"]');
            const post_type = $('[data-field="shortcode_content_post_type"]').val();

            if($(content_title).is(':checked')) {
                shortcode += ` title="1"`;
            }
