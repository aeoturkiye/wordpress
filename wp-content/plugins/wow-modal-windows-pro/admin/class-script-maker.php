...
class Script_Maker {
	...
	private function floatBtnAnimation(): void {
		if ( $this->param['umodal_button'] === 'yes' && $this->param['button_animate'] !== 'no' ) {
	...
	private function referrer(): void {
		if ( ! empty( $this->param['referrer_url_on'] ) ) {
			$this->script['referrer'] = [
				true,
				$this->param['referrer_url'],
			];
		}
	}
