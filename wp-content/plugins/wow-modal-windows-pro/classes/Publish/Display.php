...
class Display {
	...
	private static function custom_url( $i, $param ): bool {
		if ( empty( $param['custom_url'][ $i ] ) ) {
			return true;
		}
		$custom_url  = $param['custom_url'][ $i ];
		$current_url = 'http' . ( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? 's' : '' ) . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		$match       = strpos( $current_url, $custom_url );

		if ( $param['operator'][ $i ] === '1' && $match !== false ) {
			return true;
		}

		if ( $param['operator'][ $i ] === '0' && $match === false ) {
			return true;
		}
	...
	private static function url_referrer( $i, $param ): bool {
		if ( empty( $param['custom_url'][ $i ] ) ) {
			return true;
		}
		...
		$match        = strpos( $referrer_url, $custom_url );
