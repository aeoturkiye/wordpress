...
namespace ModalWindowPro\Update;
...
class UpdateDB {
	...
	public static function update_options(): void {
		...
		$license = get_option( 'wow_license_key_mwp' );
		$status  = get_option( 'wow_license_status_mwp' );
		if ( $license !== false ) {
	...
	public static function update_param( $param ) {
		if ( ! isset( $param['language_on'] ) ) {
			$param['language_on'] = $param['param[depending_language]'] ?? 0;
			$param['language'] = $param['param[lang]'] ?? '';
		}
		...
		if ( ! is_array( $param['weekday'] ) ) {
			...
			$dates_old      = ! empty( $param['set_dates'] ) ? 'enabled' : 0;
