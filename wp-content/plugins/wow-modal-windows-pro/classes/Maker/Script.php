...
class Script {
	...
	public function init(): array {
		...
		$close_delay                 = isset( $this->param['close_delay'] ) ? absint( $this->param['close_delay'] ) : 0;
		...
		if ( ! empty( $this->param['open_selectors'] ) ) {
			$this->script['openSelectors'] = ( $this->param['open_selectors'] );
		}

		if ( ! empty( $this->param['close_selectors'] ) ) {
			$this->script['closeSelectors'] = ( $this->param['close_selectors'] );
		}
		...
		if ( ! empty( $this->param['open_hash_url_on'] ) ) {
			$this->script['hash'] = [
				true,
				!empty($this->param['open_hash_url']) ? $this->param['open_hash_url'] : '#',
			];
		}
	...
	private function auto_close(): void {
		if ( ! empty( $this->param['modal_auto_close'] ) ) {
			$this->script['autoClose'] = [
				true,
				absint( $this->param['auto_close_delay'] ),
			];
		}
	}

	private function video(): void {
		if ( isset( $this->param['video_support'] ) && $this->param['video_support'] === '2' ) {
			$autoplay              = $this->param['video_autoplay'] === '2';
			$close                 = $this->param['video_close'] === '2';
			$this->script['video'] = [
				true,
				$autoplay,
				$close
	...
	private function floatBtnAnimation(): void {
		if ( $this->param['umodal_button'] === 'yes' && isset( $this->param['button_animate'] ) && $this->param['button_animate'] !== 'no' ) {
			$this->script['floatBtnAnimation'] = [
				true,
				'_' . $this->param['button_animate'],
				$this->param['button_animate_time'],
				'has-animation',
				$this->param['button_animate_pause'],
			];
		}
	}
	...
	private function cookies(): void {
		if ( $this->param['use_cookies'] === 'yes' ) {
			$timing = $this->param['set_cookies'] ?? 'open';
			$this->script['cookie'] = [
				true,
	...
	private function close_redirect(): void {
		if ( ! empty( $this->param['close_redirect_checkbox'] ) ) {
			$this->script['closeRedirect'] = [
				true,
				$this->param['close_redirect'],
				$this->param['close_redirect_target'],
			];
		}
	}

	private function byURL(): void {
		if ( ! empty( $this->param['check_popup_url'] ) ) {
			$this->script['byURL'] = [
				true,
				$this->param['open_popup_url'],
			];
		}
	}

	private function geotargeting(): void {
		if ( ! empty( $this->param['geotargeting_on'] ) ) {
			$this->script['geotargeting'] = true;
			$this->script['countries']    = $this->param['geotargeting'];
		}
	}

	private function tracking(): void {
		if ( ! empty( $this->param['enable_tracking_open'] ) ) {
			$action                       = ! empty( $this->param['event_action_open'] ) ? $this->param['event_action_open'] : 'Modal Open';
			$category                     = ! empty( $this->param['event_category_open'] ) ? $this->param['event_category_open'] : 'Modal Window';
			$label                        = ! empty( $this->param['enable_event_label_open'] ) ? $this->param['event_label_open'] : '';
			$value                        = ! empty( $this->param['enable_event_value_open'] ) ? $this->param['event_value_open'] : '';
			$this->script['trackingOpen'] = [
				true,
				$action,
				$category,
				$label,
		...
		if ( ! empty( $this->param['enable_tracking_close'] ) ) {
			$action                        = ! empty( $this->param['event_action_close'] ) ? $this->param['event_action_close'] : 'Modal Close';
			$category                      = ! empty( $this->param['event_category_close'] ) ? $this->param['event_category_close'] : 'Modal Window';
			$label                         = ! empty( $this->param['enable_event_label_close'] ) ? $this->param['event_label_close'] : '';
			$value                         = ! empty( $this->param['enable_event_value_close'] ) ? $this->param['event_value_close'] : '';
			$this->script['trackingClose'] = [
				true,
				$action,
				$category,
				$label,
